...
/** Wide horizontal card — compact height, logo left, text center, CTA right */
function FeaturedCard({ card }: { card: FeaturedCard }) {
  return (
    <div className="flex items-center gap-4 rounded-lg border border-edge bg-surface px-4 py-3.5">
      {/* Logo */}
      <a
        href={card.href}
        target="_blank"
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-[11px] font-bold text-copy-dim">{card.school}</p>
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="ml-2 shrink-0 inline-flex items-center gap-1 text-xs font-semibold text-accent hover:underline whitespace-nowrap"
      >
        Learn More
              ...
              <ScrollReveal key={prog.rank} delay={(Math.min((i % 4) + 1, 4)) as 1 | 2 | 3 | 4}>
                <div className={`flex flex-col gap-6 sm:flex-row sm:items-start ${i > 0 ? "border-t border-edge pt-12" : ""}`}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {/* Matches site-standard accent button exactly */}
                    <a
                      href={prog.href}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="mt-5 inline-flex items-center gap-2 rounded-lg bg-accent px-5 py-2.5 text-sm font-semibold text-white btn-shadow hover:bg-accent-hover"
                    >
                      Learn More
