...
const appStoreBadge = (
  <a
    ...
    className="inline-flex items-center gap-2.5 rounded-lg bg-black px-6 py-3 text-white btn-shadow transition-transform hover:scale-[1.02]"
  >
            ...
            {steps.map((s, i) => (
              <ScrollReveal key={s.step} delay={((i % 3) + 1) as 1 | 2 | 3}>
                <div className="flex h-full flex-col items-center rounded-card border border-edge bg-canvas p-6 text-center shadow-card">
                  <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[var(--accent-glow)] text-sm font-extrabold text-accent">
                    {s.step}
                  </div>
